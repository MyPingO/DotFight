...
public class AIFireAbilities : AIDotAbilitiesBase
{
	...
	private IEnumerator LeaveFireTrail()
	{
		...
		{
			if (aiMovement.IsPlayerGrounded() && !IsTouchingFireTrail())
			{
				SpawnFireTrail();
	...
	{
		RaycastHit2D groundRaycastHit = aiMovement.GetGroundRaycast();
		if (groundRaycastHit)
		{
